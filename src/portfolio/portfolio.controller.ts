...
export class PortfolioController {
  ...
  async create(@UploadedFile() image: Express.Multer.File, @Body() createPortfolioDto: CreatePortfolioDto): Promise<any> {
    const imageUrl = await this.cloudinaryService.uploadImage(image, cloudinaryFolders.portfolio, 550, 300);
    const savedData = {
  ...
  async update(@UploadedFile() image: Express.Multer.File, @Param('id') id: string, @Body() updatePortfolioDto: UpdatePortfolioDto) {
    ...
    const imageUrl = await this.cloudinaryService.updateImage(publicId.split("/")[1], image, cloudinaryFolders.portfolio, 550, 300);
    const savedData = {
